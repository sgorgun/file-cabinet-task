...
Все изменения (commits) должны храниться в ветке _step5-refactoring_, а после окончания работы должны быть слиты в ветку _main_ с помощью *squash* merge (все комментарии ветки должны быть слиты вместе).
